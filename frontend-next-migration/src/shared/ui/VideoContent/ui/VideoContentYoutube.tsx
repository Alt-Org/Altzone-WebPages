        ...
        if (!this.state.firstLoad) {
            ...
            if (src.indexOf('youtube') !== -1 && src.indexOf('watch') !== -1) {
                ...
                try {
                    ...
                    try {
                        // @ts-ignore
                        localState.videoId = new URLSearchParams(
                            new URL('https://' + src).search,
                        ).get('v');
                    } catch {
                        return <div>error loading, wrong URL...</div>;
                    }
                }
            }
            ...
            if (src.indexOf('youtube') !== -1 && src.indexOf('embed') !== -1) {
                ...
                try {
                    ...
                    try {
                        localState.videoId = localState.videoId = new URL(
                            'https://' + src,
                    ...
                    src={
                        this.state.autoPlay
                            ? 'https://www.youtube.com/embed/' +
                              this.state.videoId +
                              '?rel=0&showinfo=false&autoplay=1'
                            : 'https://www.youtube.com/embed/' +
                              this.state.videoId +
                              '?rel=0&showinfo=false'
                    }
