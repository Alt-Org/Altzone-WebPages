...
const AdminNewsElementPage = () => {
    ...
    const handleNextNews = (newsId: string) => {
        if (newsId) router.push(`/admin/news/${newsId}`);
    };
    ...
    const getImageSrc = (pictureId: string | undefined) =>
        pictureId ? `${directusBaseUrl}/assets/${pictureId}` : undefined;

    const renderNewsCard = (news: ReturnType<typeof formatNews>[0]) => (
        <NewsCard
            key={news.id}
            titlePicture={getImageSrc(news.titlePicture?.id)}
            title={news.title}
    ...
    return (
        <Container>
            ...
            <div className={cls.newsGrid}>{groupedNews.map(renderNewsCard)}</div>
        </Container>
    );
};

export default AdminNewsElementPage;
